...
@Controller
@RequestMapping("/painel/professor/perfil")
class PainelProfessorPerfilController(val usuarioRepository: UsuarioRepository) {

    @GetMapping()
    fun indexPerfil(model: Model, @AuthenticationPrincipal usuarioAutenticado: Usuario?) : String {
        model.addAttribute("usuario", usuarioAutenticado)
        return "/painel/professor/perfil/index"
    }

    @PostMapping()
    fun perfilSalvar(model: Model, @AuthenticationPrincipal usuarioAutenticado: Usuario?, @Valid usuario : Professor,
        ...
        return "redirect:/painel/professor/perfil"
    }
